export function fixtureForNew(host, options = {}) {
  ...
  const script = `
    ...
    for (const send of sends) send.addEventListener('click', event => {
      ...
      if (!options.noClear) { setText(''); grokText = ''; }
      editor?.dispatchEvent(new Event('input', { bubbles: true }));
    });
    if (host === 'grok.com' && !options.noEditorApi) {
      editor.editor = {
        view: { dom: editor }, isEditable: true,
        getText: () => grokText,
        commands: {
          focus() {
            editor.focus();
            if (options.focusDraft) { grokText = options.focusDraft; setText(grokText); }
            if (options.focusNavigate) history.pushState({}, '', options.focusNavigate);
            return true;
          },
          clearContent() { throw new Error('Clearing a composer is forbidden.'); },
          insertContent(value) {
            if (!Array.isArray(value) || value.some(item => item.type !== 'paragraph' || !Array.isArray(item.content) || item.content.some(node => node.type !== 'text' || typeof node.text !== 'string'))) return false;
            window.__tiptapCalls += 1;
            window.__tiptapPayload = structuredClone(value);
            grokText = value.map(item => item.content.map(node => node.text).join('')).join('\\n');
            setText(grokText);
            editor.dispatchEvent(new Event('input', { bubbles: true }));
            return true;
          },
    ...
    updateSend();
  `;
  return `<!doctype html><html><head><style>main{display:block;width:700px;min-height:240px;padding:24px}form{display:block;width:650px;min-height:150px}input-container,rich-textarea{display:block}.tiptap,.ql-editor,[contenteditable=true],textarea{display:block;width:500px;min-height:48px;border:1px solid #555;padding:12px}button{display:block;min-width:90px;min-height:30px;margin:8px}.send-button-container,.controls>div,.message-input-right-button-send{display:block;width:100px;min-height:30px}svg{width:16px;height:16px}user-query,.chat-content-item-user,.qwen-chat-message-user,.user-message{display:block;margin:10px;padding:8px;border:1px solid #aaa}</style></head><body><main>${config.editor}<div id="messages"></div></main><script>${script}</script></body></html>`;
}
...
const fixtures = {
  ...
  'grok.com': {
    editor: '<form><div class="tiptap ProseMirror" contenteditable="true"></div><button data-testid="chat-submit"></button></form>', editorQuery: '.tiptap.ProseMirror', sendQuery: '[data-testid="chat-submit"]', user: "const node=document.createElement('div');node.dataset.testid='user-message';node.textContent=read();document.querySelector('#messages').append(node);",
  },
  'chat.deepseek.com': {
